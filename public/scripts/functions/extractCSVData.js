...
async function initializeDB() {
  ...
  await sqlWorker.send({
    id: sqlWorker.id++,
    action: "exec",
    sql: `CREATE TABLE IF NOT EXISTS TopicsClicks (
                    Id INTEGER PRIMARY KEY AUTOINCREMENT,
                    TopicId TEXT,
...
async function extractTopics(details) {
  ...
  let topics = Object.values(
    details
      .filter((e) => e.EventName == "Launch_TopicClick")
      .reduce((acc, e) => {
        if (!acc[e.TagId]) {
          acc[e.TagId] = {
            TagName: e.TagName,
  ...
  topics.forEach((singleTopic) => {
    insertion.push(
      sqlWorker.send({
        ...
        action: "exec",
        sql:
          "INSERT INTO Topics (Id, Name, Type, SceneId) VALUES ($id, $name, $type, $sceneId)",
        params: {
          $id: singleTopic.TagId,
          $name: singleTopic.TagName,
          $type: singleTopic.TagType,
          $sceneId: singleTopic.SceneId,
        },
      })
    );
  });
    ...
    insertion.push(
      sqlWorker.send({
        ...
        params: {
          ...
          $endTime: singleSession.actions[
            singleSession.actions.length - 1
          ].EventTime.getTime(),
        },
      })
    );
...
async function extractTopicClicks(details) {
  ...
  details
    .filter((e) => e.EventName == "Launch_TopicClick")
    .forEach((singleTopicClick) => {
      insertion.push(
        sqlWorker.send({
          id: sqlWorker.id++,
          action: "exec",
          sql:
            "INSERT INTO TopicsClicks ( TopicId, SessionId, Timestamp) VALUES ($topicId, $sessionId, $timestamp)",
          params: {
            $topicId: singleTopicClick.TagId,
            $sessionId: singleTopicClick.SessionId,
            $timestamp: singleTopicClick.EventTime.getTime(),
          },
        })
      );
    });
